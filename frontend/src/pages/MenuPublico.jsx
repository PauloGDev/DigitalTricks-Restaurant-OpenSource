...
const buildEmptyState = (cats) =>
  (cats || []).reduce((acc, cat) => {
    acc[cat] = { items: [], page: 0, hasMore: true, loading: false, error: null };
    return acc;
  }, {});
...
const normalizeProduto = (p) => ({
  ...
  maxObservacaoChars: Number.isFinite(p?.maxObservacaoChars) ? p.maxObservacaoChars : 0,
});

const normalizeList = (arr) => (Array.isArray(arr) ? arr.map(normalizeProduto) : []);
...
const buildParamsSerializer = (params) => {
  ...
  Object.entries(params || {}).forEach(([key, value]) => {
    if (Array.isArray(value)) {
      value.forEach((item) => {
        if (item !== undefined && item !== null && item !== "") usp.append(key, String(item));
      });
      return;
    }
...
const getMenorPrecoVariacao = (produto) => {
  ...
  const precos = variacoes.map((v) => parseMoney(v?.precoPromocional ?? v?.precoOferta ?? v?.promocaoPreco ?? v?.preco ?? v?.precoBase ?? 0)).filter((v) => v > 0);
  return precos.length ? Math.min(...precos) : 0;
};

const getPreco = (produto) => {
  const precoDireto = parseMoney(produto?.preco ?? produto?.precoBase ?? produto?.precoMinimo ?? produto?.valor ?? produto?.precoVenda ?? 0);
  if (precoDireto > 0) return precoDireto;
  return getMenorPrecoVariacao(produto);
};

const getPrecoPromocional = (produto) => {
  const promocionalDireto = parseMoney(produto?.precoPromocional ?? produto?.valorPromocional ?? produto?.precoOferta ?? produto?.promocaoPreco ?? 0);
  if (promocionalDireto > 0) return promocionalDireto;
  const variacoes = Array.isArray(produto?.variacoes) ? produto.variacoes : [];
  const promocionais = variacoes.map((v) => parseMoney(v?.precoPromocional ?? v?.precoOferta ?? v?.promocaoPreco ?? 0)).filter((v) => v > 0);
...
const MenuPublico = () => {
  ...
  const { adicionarAoCarrinho, setNumeroMesa } = useCarrinho();

  // Capture mesa from URL
  useEffect(() => {
    const mesa = searchParams.get("mesa");
    if (mesa) setNumeroMesa(mesa);
  }, [searchParams, setNumeroMesa]);
  ...
  const [filtros, setFiltros] = useState({ search: "", ordenarPor: "maisVendidos" });
  ...
  const restauranteFallback = {
    nome: "Restaurante",
    capa: "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=1200&auto=format&fit=crop",
    logo: "https://images.unsplash.com/photo-1552566626-52f8b828add9?q=80&w=300&auto=format&fit=crop",
    slug: slug || "",
    avaliacao: 4.8,
  ...
  const restauranteView = { ...restauranteFallback, ...(restaurante || {}), categorias: categoriasDisponiveis };

  const categoriasVisiveis = useMemo(() => categoriasDisponiveis, [categoriasDisponiveis]);

  const produtosEmOferta = useMemo(() => {
    const todos = Object.values(porCategoria || {}).flatMap((sec) => sec?.items || []);
    ...
    return unicos.filter((p) => {
      ...
      return p?.ofertaVigente === true || p?.emOferta === true || (preco > 0 && promocional > 0 && promocional < preco);
    }).slice(0, 12);
  }, [porCategoria]);
  ...
  const fetchRestaurante = useCallback(async () => {
    try {
      ...
      setRestaurante({
        ...
        abertoAgora: typeof data.abertoAgora === "boolean" ? data.abertoAgora : true,
  ...
  const fetchCategorias = useCallback(async () => {
    try {
      ...
      try {
        const resFallback = await api.get(`/public/restaurantes/${slug}/produtos/listarFiltroShop`, {
          params: { page: 0, size: 100, ordenarPor: filtros.ordenarPor || "maisVendidos" },
  ...
  const loadCategoriaPage = useCallback(
    async (cat) => {
      const curr = porCategoriaRef.current?.[cat] || { items: [], page: 0, hasMore: true, loading: false, error: null };
      if (curr.loading || !curr.hasMore) return;
      if (!USE_MOCK && !slug) return;
      const pageToLoad = curr.page;

      setPorCategoria((prev) => ({ ...prev, [cat]: { ...(prev[cat] || curr), loading: true, error: null } }));

      try {
        if (USE_MOCK) {
          const data = mockListarFiltroShop({ page: pageToLoad, size: batchSize, categoria: cat, search: "", ordenarPor: filtros.ordenarPor || "maisVendidos" });
          ...
          setPorCategoria((prev) => {
            ...
            return { ...prev, [cat]: { ...prevCat, items: [...(prevCat.items || []), ...novos], page: pageToLoad + 1, hasMore, loading: false, error: null } };
          });
          return;
        }

        if (cancelByCat.current[cat]) cancelByCat.current[cat].cancel("Nova requisição desta categoria");
        cancelByCat.current[cat] = axios.CancelToken.source();

        const res = await api.get(`/public/restaurantes/${slug}/produtos/listarFiltroShop`, {
          params: { page: pageToLoad, size: batchSize, categoria: [cat], ordenarPor: filtros.ordenarPor || "maisVendidos" },
        ...
        let hasMore = false;
        if (Number.isFinite(totalPaginas)) hasMore = novos.length > 0 && pageToLoad + 1 < totalPaginas;
        else hasMore = novos.length === batchSize;
        ...
        setPorCategoria((prev) => ({ ...prev, [cat]: { ...(prev[cat] || curr), loading: false, error: true, hasMore: false } }));
      }
    },
  ...
  const loadSearchPage = useCallback(
    async (reset = false) => {
      ...
      try {
        ...
        if (USE_MOCK) {
          const data = mockListarFiltroShop({ page: pageToLoad, size: batchSize, categoria: undefined, search: filtros.search || "", ordenarPor: filtros.ordenarPor || "maisVendidos" });
          const novos = normalizeList(data?.produtos || []);
          const totalPaginas = Number(data?.totalPaginas || 1);
        ...
        if (cancelSearch.current) cancelSearch.current.cancel("Nova busca");
        cancelSearch.current = axios.CancelToken.source();

        const res = await api.get(`/public/restaurantes/${slug}/produtos/listarFiltroShop`, {
          params: { page: pageToLoad, size: batchSize, ...(filtros.search && { search: filtros.search }), ordenarPor: filtros.ordenarPor || "maisVendidos" },
    ...
    [USE_MOCK, api, slug, batchSize, filtros.search, filtros.ordenarPor, searchPage, searchHasMore, searchLoading, showNotification]
  );
  ...
  useEffect(() => {
    ...
    if (!categoriaAtual) setCategoriaAtual(categoriasDisponiveis[0]);
  ...
  useEffect(() => {
    ...
    Object.values(cancelByCat.current || {}).forEach((src) => {
      try { src?.cancel?.("Reset"); } catch {}
    });
    cancelByCat.current = {};

    const cats = categoriasVisiveis;
    if (!cats || cats.length === 0) { setPorCategoria({}); setLoadingInicial(false); return; }

    const nextState = buildEmptyState(cats);
    porCategoriaRef.current = nextState;
    setPorCategoria(nextState);

    Promise.allSettled(cats.map((cat) => loadCategoriaPage(cat))).finally(() => setLoadingInicial(false));
  }, [categoriasDisponiveis, categoriasVisiveis, filtros.ordenarPor, isSearchMode, batchSize, loadCategoriaPage, loadSearchPage]);
  ...
  useEffect(() => {
    ...
    }, { root: null, rootMargin: "-120px 0px -55% 0px", threshold: [0.1, 0.2, 0.35, 0.5, 0.7] });
  ...
  const handleAdicionar = async (payloadOrProdutoId, variacaoId = null, quantidade = 1) => {
    try {
      if (!slug && !USE_MOCK) {
        showNotification("Restaurante não identificado.", "error");
        return;
      ...
      <div className="min-h-screen bg-zinc-50 pb-24 md:pb-10">
        <div className="mx-auto max-w-3xl px-4 py-24">
          <div className="rounded-3xl border border-zinc-200 bg-white p-10 text-center shadow-sm">
            <h1 className="mb-3 text-2xl font-black text-zinc-900">Restaurante não encontrado</h1>
            <p className="mb-6 text-zinc-500">Verifique o link do cardápio ou tente novamente mais tarde.</p>
            <button onClick={() => navigate("/")} className="rounded-2xl bg-zinc-900 px-5 py-3 font-semibold text-white">Voltar ao início</button>
          </div>
        </div>
      </div>
    );
        ...
        {isSearchMode ? (
          <div className="space-y-5">
            <div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5">
              <p className="text-sm text-zinc-500">Resultados para <span className="font-bold text-zinc-900">"{filtros.search}"</span></p>
            </div>

            {loadingInicial && searchList.length === 0 ? (
              <div className="rounded-3xl border border-zinc-200 bg-white py-16 text-center text-zinc-500 shadow-sm">Carregando…</div>
            ) : searchList.length === 0 ? (
              <div className="flex items-center justify-center gap-2 rounded-3xl border border-zinc-200 bg-white py-16 text-center text-zinc-600 shadow-sm">
                <XCircle className="h-6 w-6" /> Nenhum item encontrado.
              </div>
            ) : (
              <motion.div variants={containerVariants} initial="initial" animate="animate" exit="exit" className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-5 xl:grid-cols-3">
                {searchList.map((produto) => (
                  <ProdutoCard
                    key={produto.id}
                    produto={produto}
                    variants={cardVariants}
                    onClick={() => navigate(`/restaurante/${slug}/produtos/${produto.slug}`)}
                    onAdicionar={handleAdicionar}
                  />
                ))}
              </motion.div>
            ...
            {searchLoading && <div className="py-6 text-center text-zinc-500">Carregando mais…</div>}
            {!searchHasMore && searchList.length > 0 && (
              <p className="py-6 text-center text-sm text-zinc-400">Você chegou ao fim dos resultados.</p>
            )}
          </div>
        ) : (
          <div className="space-y-10">
            {loadingInicial && categoriasVisiveis.length === 0 ? (
              ...
              <div className="rounded-3xl border border-zinc-200 bg-white py-16 text-center text-zinc-600 shadow-sm">Nenhuma categoria disponível.</div>
            ) : (
              categoriasVisiveis.map((cat) => {
                const sec = porCategoria[cat] || { items: [], loading: false, hasMore: true, error: null };
                return (
                  <section key={cat} data-cat={cat} ref={(el) => { sectionEls.current[cat] = el; }} className="scroll-mt-[260px]">
                    <div className="mb-5 flex items-end justify-between gap-3">
                      <div><h2 className="text-2xl font-black tracking-tight text-zinc-900">{cat}</h2></div>
                    </div>
                    {sec.items.length === 0 && sec.loading ? (
                      <div className="rounded-3xl border border-zinc-200 bg-white px-6 py-10 text-zinc-500 shadow-sm">Carregando…</div>
                    ) : sec.items.length === 0 ? (
                    ...
                    <div ref={(el) => (sentinels.current[cat] = el)} className="h-10" />
                    {sec.loading && sec.items.length > 0 && <div className="py-6 text-center text-zinc-500">Carregando mais…</div>}
                    {!sec.hasMore && sec.items.length > 0 && <div className="pt-8"><div className="h-px w-full bg-zinc-200" /></div>}
                  </section>
                );
              })
            )}
